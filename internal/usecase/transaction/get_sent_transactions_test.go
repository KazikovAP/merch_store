...
// Тест на успешное получение исходящих транзакций пользователя.
func TestUseCase_GetSentTransactions_Success(t *testing.T) {
	...
	transactions, err := useCase.GetSentTransactions(context.Background(), userID)
...
// Тест на ошибку при получении исходящих транзакций.
func TestUseCase_GetSentTransactions_Error(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create sqlmock: %v", err)
	}
	...
	mock.ExpectQuery(`SELECT id, sender_id, receiver_id, amount, created_at FROM transactions WHERE sender_id = \$1`).
		WithArgs(userID).
